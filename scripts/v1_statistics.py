...
def compute_rtp(row):
    ...
    odds_val = row.get("Odds") if row.get("Odds") is not None else row.get("odds")
...
def main():
    ...
    df = read_csv_from_github("results/all_betting_results.csv")
    if df.empty:
    ...
    # Normalise name column — master uses 'name', V1 rows may only have 'fighter'
    if "name" not in bets.columns and "fighter" in bets.columns:
        bets["name"] = bets["fighter"]
    elif "name" not in bets.columns:
        bets["name"] = ""
    ...
    if "won" not in bets.columns:
        ...
        bets["won"] = pd.to_numeric(bets["won"], errors="coerce").fillna(0).astype(int)
    ...
    def _net(r):
        odds_val = r.get("Odds") if r.get("Odds") is not None else r.get("odds")
        dec = american_to_decimal(odds_val)
        if dec is None:
            return 0.0
        won = r.get("won", 0) == 1 or r.get("result") == "W"
        bet = int(r.get("bet_size", 0))
        return float(dec - 1) * bet if won else -float(bet)
    bets["net"] = bets.apply(_net, axis=1)
    total_bets    = len(bets)
    ...
    if "predicted_prob" in bets.columns:
        cal_bets = bets[bets["predicted_prob"].notna()].copy()
        cal_bets["pred_bucket"] = (cal_bets["predicted_prob"].astype(float) * 10).apply(int) / 10
        for bucket, grp in cal_bets.groupby("pred_bucket"):
            if len(grp) >= 3:
                calibration.append({
    ...
    print("\n[2] Pushing ufc_statistical_analysis.json…")
    write_json_to_github(
        output,
        "statistics/ufc_statistical_analysis.json",
        "Update UFC statistical analysis JSON",
    )

    # Also write the markdown report
    md_lines = [
        ...
        f"| | |",
        f"|---|---|",
        f"| H0 (breakeven) | {BREAKEVEN} |",
        f"| t-statistic | {t_stat:.3f} |",
        f"| p-value (one-sided) | {p_one:.4f} |",
        f"| Conclusion | {conclusion} |",
    ]
    md_content = "\n".join(md_lines)

    print("  Pushing ufc_statistical_analysis.md…")
    md_b64 = base64.b64encode(md_content.encode()).decode()
    md_url  = f"https://api.github.com/repos/{REPO_OWNER}/{V1_REPO}/contents/statistics/ufc_statistical_analysis.md"
    md_check = requests.get(md_url, headers=GH_HEADERS, timeout=10)
    md_sha   = md_check.json().get("sha") if md_check.status_code == 200 else None
    md_payload = {"message": "Update UFC statistical analysis report", "content": md_b64}
    if md_sha:
        md_payload["sha"] = md_sha
    md_resp = requests.put(md_url, headers=GH_HEADERS, json=md_payload, timeout=30)
    print(f"  {'✓' if md_resp.status_code in (200,201) else '✗'} GitHub: statistics/ufc_statistical_analysis.md")
