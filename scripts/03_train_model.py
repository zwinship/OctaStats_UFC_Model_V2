...
def train_dynamic_logit(matchup_df):
    ...
    # Controls for structural differences between divisions (e.g. td rates at
    # Flyweight vs Heavyweight are not directly comparable). Drop one category
    # (Lightweight) as the reference class to avoid perfect multicollinearity.
    wc_dummies   = pd.get_dummies(df["weight_class"], prefix="wc", drop_first=False)
    ref_class    = "wc_Lightweight"
    if ref_class in wc_dummies.columns:
        wc_dummies = wc_dummies.drop(columns=[ref_class])
    wc_cols = list(wc_dummies.columns)
    df = pd.concat([df.reset_index(drop=True), wc_dummies.reset_index(drop=True)], axis=1)

    feature_cols = diff_cols + style_cols + lag_hot_cols + wc_cols
