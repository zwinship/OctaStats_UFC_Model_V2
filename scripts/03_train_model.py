...
# Each style is defined by its KEY signal stats and a minimum z-score threshold.
# Classification uses a score-based approach: a fighter scores 1 point for each
# stat that clears its threshold. The style with the most points wins (ties broken
# by priority order). This prevents early-match bias (e.g. grapplers being
# swallowed by Striker just because striking is checked first).
STYLE_THRESHOLDS = {
    "Striker":   {"sig_str_att": -0.4, "total_str_att": -0.4, "head_att": -0.4, "kd": -0.4},
    "Wrestler":  {"ctrl_seconds": -0.4, "ground_att": -0.4,    "td_att": -0.4},
    "BJJ":       {"sub_att": -0.4,      "ctrl_seconds": -0.4,  "td_att": -0.4},
    "Muay_Thai": {"clinch_att": -0.4,   "body_att": -0.4,      "leg_att": -0.4,  "head_att": -0.4},
    "Sniper":    {"distance_att": -0.4, "sig_str_pct": -0.2,   "head_att": -0.4,
                  "body_att": -0.4,     "leg_att": -0.4},
}
...
def assign_fighting_styles(df, career_df):
    ...
    def classify_style(row):
        ...
        priority = ["Wrestler", "BJJ", "Muay_Thai", "Striker", "Sniper"]
