...
def train_dynamic_logit(matchup_df):
    ...
    pipeline = Pipeline([
        ...
        ("lasso_logit", LogisticRegressionCV(
            Cs=np.logspace(-3, 1, 10),  # 10 Cs plenty for LASSO; 25 too slow on CI
    ...
    print(f"  Dynamic logit: AUC={cv_auc:.4f}, best_C={best_C:.4f}, "
...
def train_finish_model(df, career_styled, matchup_df):
    ...
    pipeline = Pipeline([
        ...
        ("clf", LogisticRegressionCV(
            Cs=np.logspace(-2, 1, 8),   # finish model CV grid
            cv=StratifiedKFold(n_splits=5, shuffle=True, random_state=42),
            penalty="l2",
            solver="lbfgs",
