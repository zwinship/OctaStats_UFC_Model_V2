...
# Style criteria and thresholds (z-scores within weight class).
#
# Key design decisions:
#   Wrestler  — defined purely by takedowns + control + ground strikes (NO sub_att).
#               Gets takedowns, holds people down, grinds.
#   BJJ       — PRIMARY signal is sub_att. Also needs td_att to confirm they get
#               there, but ctrl and ground are secondary. This separates submission
#               hunters from pure wrestlers.
#   Striker   — high volume striking (sig + total) AND head strikes. KD is a bonus
#               but not required (removed from hard criteria so volume strikers qualify).
#   Muay_Thai — clinch + body + leg + head. All four needed since it's a distinct
#               combination; hit-rate allows one weak metric.
#   Sniper    — distance striking + accuracy (sig_str_pct). Fewer criteria, tighter
#               threshold on accuracy so it stays selective.
STYLE_THRESHOLDS = {
    "Striker":   {"sig_str_att": -0.35, "total_str_att": -0.35, "head_att": -0.35, "sig_str_pct": -0.5},
    "Wrestler":  {"td_att": -0.35,      "ctrl_seconds": -0.35,  "ground_att": -0.35},
    "BJJ":       {"sub_att": -0.30,     "td_att": -0.50,        "ctrl_seconds": -0.60},
    "Muay_Thai": {"clinch_att": -0.40,  "body_att": -0.40,      "leg_att": -0.40,   "head_att": -0.40},
    "Sniper":    {"distance_att": -0.35,"sig_str_pct": -0.10,   "sig_str_att": -0.50},
}
# Minimum fraction of a style's criteria that must be met to be eligible
STYLE_MIN_HIT_RATE = 0.75
...
def assign_fighting_styles(df, career_df):
    ...
    def classify_style(row):
        ...
        # Score each style: count how many of its criteria the fighter meets.
        # A style is eligible if it hits >= STYLE_MIN_HIT_RATE of its criteria.
        # The highest-scoring eligible style wins. Ties broken by priority order below.
        scores = {}
        for style, criteria in STYLE_THRESHOLDS.items():
            hits  = sum(1 for stat, thresh in criteria.items() if z(stat) >= thresh)
            total = len(criteria)
            if hits / total >= STYLE_MIN_HIT_RATE:
                scores[style] = hits / total   # hit-rate as score

        if not scores:
            return "Mixed"

        # Priority order used only to break ties in hit-rate.
        # BJJ before Wrestler so sub-hunters aren't called wrestlers.
        # Striker before Muay_Thai since Muay_Thai is a striking subset.
        priority = ["BJJ", "Wrestler", "Striker", "Muay_Thai", "Sniper"]
        best_score = max(scores.values())
        tied = [s for s in priority if s in scores and scores[s] == best_score]
...
def build_matchup_features(df, career_styled):
    ...
    for fight_url, group in df.groupby("fight_url"):
        ...
        feat = {
            ...
            # Raw fighter-A career fights (kept for symmetric >=3 filter in training)
            "career_fights_before": float(row_a.get("career_fights_before", np.nan)),
        ...
        _ip_diff   = feat.get("diff_implied_prob", 0) or 0
        _ip_a      = feat.get("implied_prob_a", 0.5) or 0.5
        _ip_b      = feat.get("implied_prob_b", 0.5) or 0.5
        _reach     = feat.get("diff_reach_cms", 0) or 0
        _height    = feat.get("diff_height_cms", 0) or 0
        _age       = feat.get("diff_age", 0) or 0
        _ws        = feat.get("diff_current_win_streak", 0) or 0
        _ls        = feat.get("diff_current_lose_streak", 0) or 0
        _rank_wc   = feat.get("diff_fighter_rank_wc", 0) or 0
        _rank_pfp  = feat.get("diff_fighter_rank_pfp", 0) or 0
        _kd        = feat.get("diff_career_avg_kd", 0) or 0
        _sig       = feat.get("diff_career_avg_sig_str_landed", 0) or 0
        _sig_pct   = feat.get("diff_career_avg_sig_str_pct", 0) or 0
        _td        = feat.get("diff_career_avg_td_landed", 0) or 0
        _td_pct    = feat.get("diff_career_avg_td_pct", 0) or 0
        _sub       = feat.get("diff_career_avg_sub_att", 0) or 0
        _ctrl      = feat.get("diff_career_avg_ctrl_seconds", 0) or 0
        _ko_wins   = feat.get("diff_career_win_by_ko", 0) or 0
        _sub_wins  = feat.get("diff_career_win_by_sub", 0) or 0
        _dec_wins  = feat.get("diff_career_win_by_dec", 0) or 0
        _scorecard = feat.get("diff_avg_scorecard_margin", 0) or 0
        _fights    = feat.get("diff_career_fights_before", 0) or 0
        _wins      = feat.get("diff_career_wins", 0) or 0
        _losses    = feat.get("diff_career_losses", 0) or 0
        _n_rounds  = feat.get("number_of_rounds", 3) or 3
        _title     = feat.get("title_bout", 0) or 0

        # ── Win rate (wins / fights, avoids divide-by-zero) ───────────────────
        _cfb_a = float(row_a.get("career_fights_before", 0) or 0)
        _cfb_b = float(row_b.get("career_fights_before", 0) or 0)
        _cw_a  = float(row_a.get("career_wins", 0) or 0)
        _cw_b  = float(row_b.get("career_wins", 0) or 0)
        _wr_a  = _cw_a / _cfb_a if _cfb_a > 0 else np.nan
        _wr_b  = _cw_b / _cfb_b if _cfb_b > 0 else np.nan
        feat["win_rate_a"]      = _wr_a
        feat["win_rate_b"]      = _wr_b
        feat["diff_win_rate"]   = (_wr_a - _wr_b) if not (np.isnan(_wr_a) or np.isnan(_wr_b)) else np.nan

        # KO-rate and sub-rate (finish rate per career fight)
        _kor_a = float(row_a.get("career_win_by_ko",  0) or 0) / _cfb_a if _cfb_a > 0 else np.nan
        _kor_b = float(row_b.get("career_win_by_ko",  0) or 0) / _cfb_b if _cfb_b > 0 else np.nan
        _subr_a = float(row_a.get("career_win_by_sub", 0) or 0) / _cfb_a if _cfb_a > 0 else np.nan
        _subr_b = float(row_b.get("career_win_by_sub", 0) or 0) / _cfb_b if _cfb_b > 0 else np.nan
        feat["diff_ko_rate"]    = (_kor_a - _kor_b)   if not (np.isnan(_kor_a)  or np.isnan(_kor_b))  else np.nan
        feat["diff_sub_rate"]   = (_subr_a - _subr_b) if not (np.isnan(_subr_a) or np.isnan(_subr_b)) else np.nan

        # ── Log-odds of implied prob (linearises the logit signal) ────────────
        def _safe_log_odds(p):
            p = max(0.02, min(0.98, p))
            return np.log(p / (1 - p))
        feat["log_odds_ip_a"]   = _safe_log_odds(_ip_a)
        feat["log_odds_ip_b"]   = _safe_log_odds(_ip_b)
        feat["diff_log_odds_ip"] = feat["log_odds_ip_a"] - feat["log_odds_ip_b"]

        # ── Squared / polynomial terms ────────────────────────────────────────
        # Non-linear reach / height effects
        feat["diff_reach_sq"]   = _reach ** 2
        feat["diff_height_sq"]  = _height ** 2
        feat["diff_age_sq"]     = _age ** 2
        # Non-linear rank effect (being ranked #1 vs #5 >> #10 vs #14)
        feat["diff_rank_wc_sq"] = _rank_wc ** 2
        # Non-linear odds — captures blowout favourite situations
        feat["diff_ip_sq"]      = _ip_diff ** 2
        feat["log_odds_diff_sq"] = feat["diff_log_odds_ip"] ** 2

        # ── Career momentum interactions ───────────────────────────────────────
        # Market × win streak (does market correctly price hot streaks?)
        feat["odds_x_win_streak"]     = _ip_diff * _ws
        feat["odds_x_lose_streak"]    = _ip_diff * _ls
        # Win streak × rank (ranked fighter on hot streak)
        feat["rank_x_win_streak"]     = _rank_wc * _ws
        # Win rate × implied prob (does market track career quality?)
        _dwr = feat.get("diff_win_rate", 0) or 0
        feat["ip_x_win_rate"]         = _ip_diff * _dwr

        # ── Physical attribute interactions ───────────────────────────────────
        feat["reach_x_ip"]            = _reach * _ip_diff
        feat["height_x_ip"]           = _height * _ip_diff
        feat["reach_x_sig"]           = _reach * _sig
        feat["reach_x_kd"]            = _reach * _kd
        feat["reach_x_rank"]          = _reach * _rank_wc
        # Age interactions (prime vs declining fighter)
        feat["age_x_ip"]              = _age * _ip_diff
        feat["age_x_win_streak"]      = _age * _ws
        feat["age_x_sig"]             = _age * _sig

        # ── Striking quality interactions ─────────────────────────────────────
        # High volume + high accuracy = real striker advantage
        feat["sig_x_sig_pct"]         = _sig * _sig_pct
        feat["sig_x_kd"]              = _sig * _kd
        # KD power × implied prob (does market correctly price KO artists?)
        feat["kd_x_ip"]               = _kd * _ip_diff
        # Distance vs clinch striking
        _dist  = feat.get("diff_career_avg_distance_att", 0) or 0
        _clinch = feat.get("diff_career_avg_clinch_att", 0) or 0
        feat["dist_x_clinch"]         = _dist * _clinch

        # ── Grappling interactions ────────────────────────────────────────────
        feat["td_x_ctrl"]             = _td * _ctrl
        feat["td_x_sub"]              = _td * _sub
        feat["td_pct_x_ctrl"]         = _td_pct * _ctrl
        feat["sub_x_ctrl"]            = _sub * _ctrl
        # Grappling vs striking trade-off
        feat["td_x_sig"]              = _td * _sig
        feat["ctrl_x_sig"]            = _ctrl * _sig

        # ── Finish style × career record ──────────────────────────────────────
        feat["ko_x_scorecard"]        = _ko_wins * _scorecard
        feat["sub_x_scorecard"]       = _sub_wins * _scorecard
        feat["ko_rate_x_reach"]       = (_kor_a - _kor_b if not (np.isnan(_kor_a) or np.isnan(_kor_b)) else 0) * _reach

        # ── Title bout context ────────────────────────────────────────────────
        feat["title_x_fights"]        = _title * _fights
        feat["title_x_rank"]          = _title * _rank_wc
        feat["title_x_ip"]            = _title * _ip_diff
        feat["title_x_win_streak"]    = _title * _ws
        feat["title_x_age"]           = _title * _age

        # ── Round count interactions (5-rounders are different fights) ────────
        feat["rounds_x_ctrl"]         = _n_rounds * _ctrl
        feat["rounds_x_td"]           = _n_rounds * _td
        feat["rounds_x_ip"]           = _n_rounds * _ip_diff
        feat["rounds_x_sub"]          = _n_rounds * _sub

        # ── Ranking interactions ───────────────────────────────────────────────
        feat["rank_wc_x_ip"]          = _rank_wc * _ip_diff
        feat["rank_pfp_x_ip"]         = _rank_pfp * _ip_diff
        feat["rank_wc_x_sig"]         = _rank_wc * _sig
        feat["rank_wc_x_kd"]          = _rank_wc * _kd
        feat["rank_wc_x_td"]          = _rank_wc * _td

        # ── Experience gap interactions ────────────────────────────────────────
        # Veteran vs newcomer — does experience edge interact with other signals?
        feat["fights_x_ip"]           = _fights * _ip_diff
        feat["fights_x_win_streak"]   = _fights * _ws
        feat["fights_x_sig"]          = _fights * _sig
...
def train_dynamic_logit(matchup_df):
    ...
    # All engineered absolute / interaction / polynomial features
    # (diff_ cols are captured separately above; these are non-diff engineered features)
    abs_feat_cols  = [
        # Absolute market signals
        "implied_prob_a", "implied_prob_b", "implied_prob_source_a",
        # Log-odds transform of implied prob (linearises logit signal)
        "log_odds_ip_a", "log_odds_ip_b", "diff_log_odds_ip",
        # Context flags
        "title_bout", "empty_arena", "number_of_rounds",
        # Win / finish rates
        "win_rate_a", "win_rate_b",
        # Polynomial / squared terms
        "diff_reach_sq", "diff_height_sq", "diff_age_sq",
        "diff_rank_wc_sq", "diff_ip_sq", "log_odds_diff_sq",
        # Career momentum interactions
        "odds_x_win_streak", "odds_x_lose_streak", "rank_x_win_streak", "ip_x_win_rate",
        # Physical × performance interactions
        "reach_x_ip", "height_x_ip", "reach_x_sig", "reach_x_kd", "reach_x_rank",
        "age_x_ip", "age_x_win_streak", "age_x_sig",
        # Striking quality interactions
        "sig_x_sig_pct", "sig_x_kd", "kd_x_ip", "dist_x_clinch",
        # Grappling interactions
        "td_x_ctrl", "td_x_sub", "td_pct_x_ctrl", "sub_x_ctrl",
        "td_x_sig", "ctrl_x_sig",
        # Finish style × record
        "ko_x_scorecard", "sub_x_scorecard", "ko_rate_x_reach",
        # Title bout context
        "title_x_fights", "title_x_rank", "title_x_ip",
        "title_x_win_streak", "title_x_age",
        # Round count interactions
        "rounds_x_ctrl", "rounds_x_td", "rounds_x_ip", "rounds_x_sub",
        # Ranking interactions
        "rank_wc_x_ip", "rank_pfp_x_ip", "rank_wc_x_sig",
        "rank_wc_x_kd", "rank_wc_x_td",
        # Experience gap interactions
        "fights_x_ip", "fights_x_win_streak", "fights_x_sig",
    ]
    ...
    # Require BOTH fighters to have >= 3 completed fights.
    # career_fights_before counts fights before the current row, so
    # completed = career_fights_before + 1. Mirrors predict-time filter exactly.
    if "career_fights_before" in model_df.columns and "diff_career_fights_before" in model_df.columns:
        cfb_a = model_df["career_fights_before"].fillna(0) + 1
        cfb_b = (model_df["career_fights_before"].fillna(0) - model_df["diff_career_fights_before"].fillna(0)) + 1
        model_df = model_df[(cfb_a >= 3) & (cfb_b >= 3)]
    elif "career_fights_before" in model_df.columns:
        model_df = model_df[(model_df["career_fights_before"].fillna(0) + 1) >= 3]
