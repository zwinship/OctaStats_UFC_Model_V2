...
def train_dynamic_logit(matchup_df):
    ...
    # Require BOTH fighters to have >= 3 fights — mirrors the prediction-time filter
    # so training and inference see the same data quality floor symmetrically.
    if "career_fights_before" in model_df.columns and "diff_career_fights_before" in model_df.columns:
        cfb_a = model_df["career_fights_before"].fillna(0)
        # fighter_b career_fights_before = fighter_a's value minus the diff
        cfb_b = cfb_a - model_df["diff_career_fights_before"].fillna(0)
        model_df = model_df[(cfb_a >= 3) & (cfb_b >= 3)]
    elif "career_fights_before" in model_df.columns:
        model_df = model_df[model_df["career_fights_before"].fillna(0) >= 3]
