...
SCRAPE_HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; OctaStats/2.0)"}
...
def get_winners_from_event(event_url):
    """
    Return a set of winner names from a completed event page.
    We take the first fighter from each row (index 0) — these are the winners
    when sorted by the default UFC stats ordering (winner is always listed first).
    """
    ...
    for row in soup.select(".b-fight-details__table-row.b-fight-details__table-row__hover"):
        # Each row has two fighters; the winner is marked with a "win" class
        # Try to find the win indicator
        cols = row.select(".b-fight-details__table-col")
        if len(cols) < 2:
            continue

        # The winner is the fighter whose name appears with a "b-flag_style_green" class
        # or we fall back to scraping the individual fight pages
        win_flags = row.select(".b-fight-details__table-col:first-child .b-flag_style_green")
        if win_flags:
            name_links = row.select(".b-fight-details__table-col:first-child .b-link")
            if name_links:
                winner_names.add(name_links[0].get_text(strip=True))
        else:
